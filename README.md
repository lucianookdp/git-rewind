...
Built it because the usual "developer wrapped" idea needs a login and a backend it doesn't, and it's usually a snapshot of a fixed recent window when the actually interesting story is the whole account. GitHub's public REST API is enough on its own: no token, no server, no database, and everything shown spans the account's real history, not the last N days.
